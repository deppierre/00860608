...
for(d=0;d<dbNames.length;d++){
    collectionNames = db.getSiblingDB(dbNames[d]).getCollectionInfos({"type": "collection"},true);
...
dbindex.restaurants                                                     183     0       0       0       0       0       0
         _id_                                                           10      0       0       0       0       0       0
dbnoindex.restaurants                                                   183     0       0       0       0       0       0
         _id_                                                           10      0       0       0       0       0       0                                                182     2       1       0       0       0       26

db.restaurants.createIndex({"name":1,"address.city":1});
db.restaurants.find({"name": "Boulevard Bistro"});

indexDetails

mlaunch init --single --wiredTigerCacheSizeGB 0.25 &&\
mongoimport -d dbindex restaurants.json &&\
mongoimport -d dbnoindex restaurants.json &&\
mongo dbindex --eval 'db.restaurants.createIndex({"name":1,"address.city":1});'

mongo --quiet "mongodb://localhost:27017" 2_page_in_cache.js

mongo 4_apps/cache.js

163904  data/db/collection-7-6143387711744107972.wt



db.restaurants.stats().wiredTiger

mongo dbindex --eval 'db.restaurants.find({"name": "Boulevard Bistro","address.city":"Onkaczu"});'
mongo dbindex --eval 'db.restaurants.stats({indexDetails:true,indexDetailsName:"name_1_address.city_1"});'
mongo dbnoindex --eval 'db.restaurants.find({"name": "Boulevard Bistro","address.city":"Onkaczu"});'
mongo dbnoindex --eval 'db.restaurants.createIndex({"name":1});'
...
mongo localhost:27020/dbindex --eval 'db.restaurants.find({"name": "Boulevard Bistro"});'
